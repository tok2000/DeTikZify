...
from detikzify.cambrian.infer import DetikzifyPipeline, TikzDocument
from detikzify.cambrian.model.cambrian_c3 import load as load_model
...
def parse_args():
    ...
    argument_parser.add_argument(
        "--max_tries",
        type=int,
        help="max number of generation proposals to try per image before giving up (early-stop if still not compilable)"
    )
...
def generate(pipe, image, strict=False, timeout=None, max_tries=None, **tqdm_kwargs):
    """Run MCTS until the generated tikz code compiles."""
    start, success, tikzpics = time(), False, set()
    for i, (score, tikzpic) in enumerate(tqdm(pipe.simulate(image=image), desc="Try", **tqdm_kwargs), start=1):
        ...
        # stop generating after max_tries unsuccessful tries
        if (not success) and max_tries is not None and i >= max_tries:
            break
    return [tikzpic for _, tikzpic in sorted(tikzpics, key=itemgetter(0))]

def predict(model_name, base_model, testset, cache_file=None, timeout=None, max_tries=None, key="image"):
    ...
    try:
        ...
        for item in tqdm(worker_chunk, desc=f"{model_name.title()} ({RANK})", disable=RANK!=0):
            tikz = generate(pipe, image=item[key], timeout=timeout, max_tries=max_tries, position=1, leave=False, disable=RANK!=0)
...
def load_metrics(trainset, measure_throughput=False, **kwargs):
    ...
    def compute(references, predictions):
        ...
        valid_mask = [pred[-1].is_rasterizable for pred in predictions]
