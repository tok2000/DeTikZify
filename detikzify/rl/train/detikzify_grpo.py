...
from detikzify.cambrian.model.cambrian_v3.cambrian_llama import print_mem
...
class DetikzifyGRPOTrainer(GRPOTrainer):
    def __init__(
        self,
        ...
        # Processing class
        if processing_class is None:
            processing_class = AutoProcessor.from_pretrained(model.config._name_or_path) # CHANGED from AutoTokenizer
        if processing_class.tokenizer.pad_token is None: # CHANGED from processing_class to .tokenizer
        ...
        for i, (reward_processing_class, reward_func) in enumerate(zip(reward_processing_classes, reward_funcs)):
            if isinstance(reward_func, PreTrainedModel):
                if reward_processing_class is None:
                    reward_processing_class = AutoTokenizer.from_pretrained(reward_func.config._name_or_path) # NOT SAFE YET
        ...
        Trainer.__init__( # CHANGED from super() to Trainer
            self,
            model=model,
            args=args,
    ...
    def _set_signature_columns_if_needed(self): # NOT SAFE YET
    ...
    def get_train_dataloader(self):
        ...
        train_dataset = self.train_dataset # MAYBE train_dataset.dataset
        data_collator = self.data_collator # NOT SAFE YET
    ...
    @profiling_decorator
    def _get_per_token_logps(self, model, input_ids, attention_mask, pixel_values, logits_to_keep, batch_size=None) -> torch.Tensor: # CHANGED to accept pixel values
        batch_size = batch_size or input_ids.size(0)  # Chunk inputs into smaller batches to reduce memory peak
        all_logps = []
        for i in range(0, input_ids.size(0), batch_size):
            input_ids_batch = input_ids[i : i + batch_size]
            attention_mask_batch = attention_mask[i : i + batch_size]
            pixel_batch = pixel_values[i : i + batch_size] # ADDED to feed the model with the image pixel values

            # We add 1 to `logits_to_keep` because the last logits of the sequence is later excluded
            outputs = model( # ADDED the pixel values for model generation, DELETED the logits_to_keep part
                input_ids=input_ids_batch, attention_mask=attention_mask_batch, pixel_values=pixel_batch,
                use_cache=False, output_hidden_states=False,
            )
            
            logits = outputs.logits # ADDED
            logits = logits[:, - (logits_to_keep + 1) :, :] # ADDED to reconstruct the logits_to_keep logic
    ...
    def _generate_and_score_completions(
        ...
        #prompts = [x["prompt"] for x in inputs] # NEED FOR CHANGE
        #prompts_text = [maybe_apply_chat_template(example, self.processing_class)["prompt"] for example in inputs] # NEED FOR CHANGE
        #prompt_inputs = self.processing_class( # CHANGED
        ...
        gen_inputs = self.processing_class(images=images, return_tensors="pt", padding=True)
        ...
        ''' DEBUG logic to find out whether model produces reasonable outputs
        
        masked_completions = torch.where(
            completion_mask.bool(),
            completion_ids,
            torch.full_like(completion_ids, self.processing_class.tokenizer.pad_token_id),
        )
        ...
        print("[DEBUG]: Cleaned-up IDs:\n", masked_completions)
        print("[DEBUG]: Decoded TikZ:\n", decoded)
        '''
        ...
        with torch.no_grad():
            ...
            if self.num_iterations > 1: # CHANGED to pipeline the pixel values into function
                old_per_token_logps = self._get_per_token_logps(
                    self.model, prompt_completion_ids, attention_mask, pixel_values, logits_to_keep, batch_size
                )
            else:
                old_per_token_logps = None
                
        ''' # ADDED first, found out it is not needed
        masked_completions = torch.where( # ADDED to mask the output
            completion_mask.bool(),
            completion_ids,
            torch.full_like(completion_ids, self.processing_class.tokenizer.pad_token_id),
        )'''
        ...
        completions = self.processing_class.batch_decode(completion_ids, skip_special_tokens=True) # CHANGED from completions_text
        ...
        for i, (reward_func, reward_processing_class, reward_func_name) in enumerate(
            ...
            with profiling_context(self, reward_func_name):
                if isinstance(
                    ...
                    #print(reward_kwargs)
                    #print_mem("Before rewards")
                    output_reward_func = reward_func(prompts=prompts, completions=completions, **reward_kwargs)
                    #print_mem("After rewards")
        ...
        #self._textual_logs["prompt"].extend(gather_object(prompts_text)) # DELETED as there are no textual prompts in our case
        self._textual_logs["completion"].extend(gather_object(completions)) # CHANGED from completions_text
    ...
    def _compute_loss(self, model, inputs):
        ...
        #print_mem("Start of loss computation")
        prompt_ids, prompt_mask = inputs["prompt_ids"], inputs["prompt_mask"]
        completion_ids, completion_mask = inputs["completion_ids"], inputs["completion_mask"]
        pixel_values = inputs["pixel_values"] # ADDED to enable the model to access the pixel values of the image
        input_ids = torch.cat([prompt_ids, completion_ids], dim=1)
        attention_mask = torch.cat([prompt_mask, completion_mask], dim=1)
        logits_to_keep = completion_ids.size(1)  # we only need to compute the logits for the completion tokens

        #print_mem("Before logps")
        per_token_logps = self._get_per_token_logps(model, input_ids, attention_mask, pixel_values, logits_to_keep) # ADDED to enable the model to access the pixel values of the image

        # Compute the KL divergence between the model and the reference model
        if self.beta != 0.0:
            with torch.no_grad():
                if self.ref_model is not None:
                    ref_per_token_logps = self._get_per_token_logps( # ADDED to enable the model to access the pixel values of the image
                        self.ref_model, input_ids, attention_mask, pixel_values, logits_to_keep
                    )
                else:
                    with self.accelerator.unwrap_model(self.model).disable_adapter():
                        ref_per_token_logps = self._get_per_token_logps( # ADDED to enable the model to access the pixel values of the image
                            self.model, input_ids, attention_mask, pixel_values, logits_to_keep
                        )
            per_token_kl = (
                torch.exp(ref_per_token_logps - per_token_logps) - (ref_per_token_logps - per_token_logps) - 1
            )
        #print_mem("After logps")
