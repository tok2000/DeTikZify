...
class DetikzifyGRPOTrainer(GRPOTrainer):
    def __init__(
        ...
        for i, (reward_processing_class, reward_func) in enumerate(zip(reward_processing_classes, reward_funcs)):
            if isinstance(reward_func, PreTrainedModel):
                ...
                reward_processing_class.padding_side = "right" # changed to align with TRL
    ...
    def get_train_dataloader(self):
        ...
        dataloader_params = {
            "batch_size": self._train_batch_size * self.args.steps_per_generation,  # <- this is the change, changed back to align with TRL
    ...
    def _generate_and_score_completions(
        ...
        for i, L in enumerate(prompt_lens.tolist()):
            comp_list.append(prompt_completion_ids[i, L:])
        completion_ids = pad_sequence(comp_list, batch_first=True, padding_value=pad_id)
        
        # keep the original right-padded prompts
        ...
        for i, (reward_func, reward_processing_class, reward_func_name) in enumerate(
            ...
            with profiling_context(self, reward_func_name):
                if isinstance(
                    ...
                    completion_ids_list = [ # new to align with TRL
                        [tid.item() for tid, m in zip(row, mask_row) if m]
                        for row, mask_row in zip(completion_ids, completion_mask)
                    ]
                    output_reward_func = reward_func(
                        prompts=prompts, completions=completions, completion_ids_list=completion_ids_list, **reward_kwargs
                    )
