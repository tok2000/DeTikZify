...
class DetikzifyCambrianConfig(PretrainedConfig): # store the overall configuration of the model, including the vision and text configurations
    ...
    def __init__(
        ...
        vision_fusion_layers=[4, 8, 12], # fusion layers where SVA is injected (D)
        num_groups=3, # number of query groups (G)
        spatial_grid_size=(27, 27), # number of grid blocks the image is split into
        spatial_temp=1.0, # spatial bias temperature
        ...
        **kwargs,
        ...
        if mm_vision_tower_aux_list is None:
            mm_vision_tower_aux_list = ["google/siglip-so400m-patch14-384"]
        ...
        self.vision_fusion_layers = vision_fusion_layers
