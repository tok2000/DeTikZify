...
from .cambrian_llama import DetikzifyCambrianLlamaModel
...
class DetikzifyCambrianModel(DetikzifyPreTrainedModel): # main model class for Detikzify
    def __init__(self, config: DetikzifyCambrianConfig):
        ...
        if isinstance(config.mm_vision_tower_aux_list, list) and len(config.mm_vision_tower_aux_list) > 1:
            print(f"Initializing Vision Towers: {config.mm_vision_tower_aux_list}")  # Debugging
            self.vision_models = nn.ModuleList([])
            for vision_tower_name in config.mm_vision_tower_aux_list:
                if "CLIP-convnext" in vision_tower_name:
                    print(f"Loading Clip-ConvNext: {vision_tower_name}")
                    self.vision_models.append(CLIPConvNextTower(vision_tower_name, config))
                elif "vit" in vision_tower_name.lower():
                    print(f"Loading Clip: {vision_tower_name}")
                    self.vision_models.append(ClipVisionTower(vision_tower_name, config))
                elif "siglip" in vision_tower_name:
                    print(f"Loading Siglip: {vision_tower_name}")
                    self.vision_models.append(SiglipVisionTower(vision_tower_name, config))
                elif "dino" in vision_tower_name:
                    print(f"Loading Dino: {vision_tower_name}")
                    self.vision_models.append(DinoVisionTower(vision_tower_name, config))
                else:
                    raise ValueError(f"Unknown vision encoder: {vision_tower_name}")
        ...
        with torch.no_grad(): # infer kv_dim_list dynamically
            ...
            kv_dim_list = [out[-1] for out in encoder_outputs]
            vision_tower_token_len_list = [out[-2] for out in encoder_outputs]
            kv_grid_shapes = [encoder.get_output_grid_shape() for encoder in self.vision_models]

        if config.mm_projector_type == "sva": # if projector type is spatial vision aggregator
            sva_blocks = nn.ModuleList([
                SpatialVisionAggregator( # initialize spatial vision aggregator (SVA) module
                    kv_dim_list=kv_dim_list,
                    kv_grid_shapes=kv_grid_shapes,
                    hidden_dim=config.text_config.hidden_size, # hidden dimension
                    num_groups=config.num_groups, # number of hidden layers
                    grid_size=config.spatial_grid_size, # number of grid blocks the image is split into
                    spatial_temp=config.spatial_temp, # spatial bias temperature
                )
                for _ in range(len(config.vision_fusion_layers))
            ])
        else:
            self.connector = DetikzifyConnector(config) # initialize connector module

        llama_base_model = AutoModel.from_config(config.text_config)
        self.text_model = DetikzifyCambrianLlamaModel(
            llama_base_model,
            vision_fusion_layers=config.vision_fusion_layers,
            max_vision_tokens=self.image_seq_len,
            sva_blocks=sva_blocks
        )
    ...
    def forward( # forward pass for the Detikzify model
        ...
        if pixel_values is not None and image_hidden_states is not None:
            ...
            if self.config.mm_projector_type == "sva":
                #image_hidden_states = self.sva_blocks[0](vision_hidden_states_list) # deleted after sanity check
                sva_kwargs["vision_latents_list"] = vision_hidden_states_list
            ...
            if self.config.mm_projector_type == "sva":
                sva_kwargs["vision_latents_list"] = image_hidden_states
                image_hidden_states = None # prevent accidental fallback behavior
...
class DetikzifyCambrianForConditionalGeneration(DetikzifyPreTrainedModel, GenerationMixin): # model class for conditional generation
    ...
    def __init__(self, config):
        ...
        ''' already done in DetikzifyCambrianModel
        if len(self.model.config.mm_vision_tower_aux_list) > 1:
            with torch.no_grad():
                dummy = torch.randn(1, 3, 384, 384).to("cuda" if torch.cuda.is_available() else "cpu")
                kv_dim_list = [vm(dummy).shape[-1] for vm in self.model.vision_models]
                kv_grid_shapes = [encoder.get_output_grid_shape() for encoder in self.model.vision_models]
            self.model.sva_blocks = nn.ModuleList([
                SpatialVisionAggregator(
                    ...
                    num_groups=self.model.config.num_groups,
                    grid_size=self.model.config.spatial_grid_size,
                    spatial_temp=self.model.config.spatial_temp
                )
                for _ in range(len(config.vision_fusion_layers))
            ])
        else:
            self.model.sva_blocks = None  # Disable SVA for single encoder setups
        '''
