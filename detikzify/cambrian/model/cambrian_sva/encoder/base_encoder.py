...
class ProcessorWrapper:
    ...
    def preprocess(self, image, return_tensors='pt'):
        return self.__call__([image], return_tensors)

    def __call__(self, images, return_tensors='pt'):
        if not isinstance(images, (list, tuple)):
            images = [images]

        processed = [self._transforms(img) for img in images]  # Apply transform to each image
...
class BaseVisionTower(nn.Module, ABC):
    def __init__(self, vision_tower_name, args, delay_load=False):
        ...
        self._interp_size = 324 # reduced after 21516
    ...
    def forward(self, images):
        ...
        print(f"{self.__class__.__name__} Vision Output Shape: {image_features.shape}") # debugging output what shape the vision latents have
