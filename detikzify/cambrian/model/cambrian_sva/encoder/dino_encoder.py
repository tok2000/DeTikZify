...
class DinoVisionTower(BaseVisionTower):
    ...
    def load_model(self, device_map=None): # load the model and configure the image processor
        ...
        self.vision_tower._no_split_modules = ["Dinov2SwiGLUFFN"] # workaround for the above error
        ...
        logger.info(f"Dino Vision Processor")
        self.image_processor = processor # configure the image processor
        self._hidden_size = self.vision_tower.config.hidden_size # hidden size of the model
        self._patch_size = self.vision_tower.config.patch_size # patch size of the model
    ...
    def feature_select(self, outputs):
        ...
        if self.select_feature == 'cls_patch': # select cls token and all patch tokens
            image_features = sequence_output
        elif self.select_feature == 'patch': # select all patch tokens
            image_features = sequence_output[:, 1:]
        elif self.select_feature == 'cls': # select cls token
