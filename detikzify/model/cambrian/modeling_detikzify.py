    ...
    def __init__(self, config: DetikzifyCambrianConfig):
        ...
        if isinstance(config.mm_vision_tower_aux_list, list) and len(config.mm_vision_tower_aux_list) > 1:
            ...
            for vision_tower_name in config.mm_vision_tower_aux_list:
                ...
                elif "CLIP-convnext" in vision_tower_name:
        ...
        if config.mm_projector_type == "sva": # if projector type is spatial vision aggregator
            self.sva = SpatialVisionAggregator( # initialize spatial vision aggregator (SVA) module
                ...
                query_num_list=config.query_num_list, # number of query tokens per encoder tower
            )
        else:
    ...
    def forward( # forward pass for the Detikzify model
        ...
        if pixel_values is not None and image_hidden_states is not None:
            ...
            if self.config.mm_projector_type == "sva":  # SVA mode
                ...
                queries.requires_grad_() # after 17189
        ...
        if past_seen_tokens == 0 and inputs_embeds is not None and image_hidden_states is not None: # changed after 17179
...
class DetikzifyCambrianForConditionalGeneration(DetikzifyPreTrainedModel, GenerationMixin): # model class for conditional generation
    ...
    def __init__(self, config):
        ...
        # following lines added after 17256
        if len(self.model.config.mm_vision_tower_aux_list) > 1:
            ...
            self.model.sva = SpatialVisionAggregator(
                q_dim=self.model.config.text_config.hidden_size,
                kv_dim_list=kv_dim_list, 
                hidden_dim=self.model.config.text_config.hidden_size,
                num_heads=self.model.config.text_config.num_attention_heads,
                num_layers=self.model.config.sva_layers,
                query_num_list=self.model.config.query_num_list
            )
        else:
            self.model.sva = None  # Disable SVA for single encoder setups
